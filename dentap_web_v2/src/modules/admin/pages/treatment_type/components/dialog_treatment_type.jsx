...
export const DialogTreatmentType = () => {

    const [hasEmptyFields, setHasEmptyFields] = useState(false);
    const [loading, setLoading] = useState(false);
    const toast = useRef(null); 

    const { updateTreatmentType, saveTreatmentType } = TreatmentTypeThunk();
    ...
    const saveData = async () => {
        ...
        if (dialog.content.every(campo => campo.value)) {
            if (dialog.modificar) {
                setLoading(true);
                dispatch(updateTreatmentType(dialog.content)).then(result => {
                    console.log('Result: ', result);
                    if (result.error) {
                        showError('Error al modificar el tipo de tratamiento', result.message, 'error');
                    } else {
                        Swal.fire(
                            '¡Actualizado!',
                            'El tipo de tratamiento ha sido actualizada.',
                            'success'
                        )
                        setLoading(false);
                        hideDialog();
                ...
                setLoading(true);
                dispatch(saveTreatmentType(dialog.content)).then(result => {
                    console.log('Result: ', result);
                    if (result.error) {
                        showError('Error al guardar el tipo de tratamiento', result.message, 'error');
                    } else {
                        Swal.fire(
                            '¡Guardado!',
                            'El tipo de tratamiento ha sido guardado.',
                            'success'
                        )
                        setLoading(false);
                        hideDialog();
